...
- A **Wave** is **derived, never stored**: the topological generations of the Batch's members under the Link graph. Wave 1 = members with no pending blockers; Wave 2 = the ones unblocked once Wave 1 is `done`; and so on. `conflicts_with` pushes mutually exclusive members into different Waves (tie-break: Priority, then ID). Blockers **outside the Batch count** — one definition of "ready", shared with the frontier (§3.1). Issues in the same Wave are mutually independent: the set safe to fan out across git worktrees in parallel. `ito list --batch <name> --ready` is the current Wave. `ito batch show --include-done` keeps completed members in the derivation, so closed or partially closed Batches can still show historical Waves without storing a plan.
...
| `ito batch show <name>`      | The Batch's open members grouped by **derived Waves**, plus progress. `--include-done` includes completed members and marks historical Waves as done. A dependency cycle among members is a clear error naming the Issues involved. |
