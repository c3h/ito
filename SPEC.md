...
- **Consequence:** the CLI must be **agent-native**:
  ...
  - **no interactive prompts** that would stall an agent (the single exception is the first-run choice of bare `ito` on a TTY, §TUI).
...
| `branch`     | text            | working git branch; empty = unset                 |
| `assignee`   | text            | who implements the Issue, typically model plus effort; free text, empty = unassigned |
...
- Removal is **destructive** and has no trash. `rm` deletes the Issue and its Links/Labels in the same transaction; links from other Issues that point to the removed Issue are also deleted. No other Issue is removed by cascade. `prune` requires `--status <s>` and explicit confirmation by flag (`--yes`); without both, it fails with exit `2`. There is no interactive prompt.
...
- A **Wave** is **derived, never stored**: the topological generations of the Batch's members under the Link graph. Wave 1 = members with no pending blockers; Wave 2 = the ones unblocked once Wave 1 is `done`; and so on. `conflicts_with` pushes mutually exclusive members into different Waves (tie-break: Priority, then ID). Blockers **outside the Batch count** — one definition of "ready", shared with the frontier (§3.1). Members gated directly or transitively by a non-`done` Issue outside the Batch form a derived **waiting** group after the Waves; they never enter a numbered Wave and never disappear from `batch show`. Issues in the same Wave are mutually independent: the set safe to fan out across git worktrees in parallel. `ito list --batch <name> --ready` is the current Wave minus the members already in flight — the frontier only ever lists `backlog`/`todo`. `ito batch show --include-done` keeps completed members in the derivation, so closed or partially closed Batches can still show historical Waves without storing a plan.
...
| `ito new`           | Mints an ID + creates the issue. Requires a non-empty `--title`; accepts `--status` (default `backlog`), `--priority` (default `low`), `--category` (default `uncategorized`), `--triage-state` (default `needs-triage`), a repeatable `--label` for initial Labels, `--body "text"` or `--body -` to read stdin, `--assignee <text>` for the free-text assignee. Without `--body`, the body is empty. Prints the ID. |
| `ito edit <ID>`     | Edits an issue's fields/body (`--title`, `--priority`, `--category`, `--triage-state`, `--branch`, `--assignee`, `--body "text"` or `--body -`; `--branch ""` clears the working branch, `--assignee ""` clears the assignee, and the body always replaces the whole text) and applies repeatable incremental operations (`--add-label`, `--remove-label`, `--block`, `--unblock`, `--relate`, `--unrelate`). Requires at least one change; without changes it fails with exit `2`. Redundant operations are idempotent (exit `0`) and only stamp `updated` when the final state changes. `status` still changes through `ito move`. |
| `ito list`          | Lists issues of the current project (hides `done` by default; use `--status done` for completed Issues or `--status all` for every status, including `done`). Separate axes: **scope** `--all-projects`; **status** `--status <s>` (filters). Plus `--label`, `--priority`, `--category`, `--triage-state`, `--assignee` (exact match), `--search`, `--ready`, `--json`. `--ready` narrows to the **ready frontier** (§3.1): `backlog`/`todo` Issues whose `blocked_by` are all `done` — the set safe to fan out across worktrees. Default ordering: Status in the flow (`backlog`, `todo`, `in_progress`, `in_review`, `done` when included), Priority (`urgent`, `high`, `medium`, `low`), then `updated` desc. With `--search`, it orders by textual ranking and uses the normal ordering as a tie-breaker. With `--all-projects`, human output groups by Project; JSON returns a flat array ordered by `project`, then the normal ordering. |
...
- Human write output is minimal: `new` prints **only** the ID (so it composes in a shell) and `prune` prints only the deleted count; every other write prints one short sentence naming the Issue and whether anything changed.
- **Failure:** exit **≠ 0** + a **human, actionable message on stderr** (in both modes). The actionable sentence is the product — e.g.: `no Project registered for the current directory. run 'ito init' in this Project or use --project <name>.` It's what makes the agent take the right action (it sustains the "zero ceremony" flow).
...
- **Exit codes** (a small, stable taxonomy):
  ...
  | `3`  | not found (an Issue ID or a Batch name) |
  | `4`  | Project not resolved (cwd not registered, or `--project` names an unknown Project) |
...
- **Per command:** `list` → an **array** of issue objects; `show` → **one** object; `new`/`move`/`edit` → the **resulting** object (the agent gets the ID/new state without an extra `show`); `rm` → a summary (`{"deleted":1,"id":"AUTH-12"}`); `prune` → a summary (`{"deleted":3}`); `init`/`rename` → a Project object (`{"id":1,"name":"ito","prefix":"ITO","root_path":"/abs/path"}`). In a Project, `root_path` can be `null` when detached. `batch show` → one Batch object whose `waves` and `waiting` arrays are always present; `waiting` contains members gated by non-`done` Issues outside the Batch and is `[]` when none exist.
- An empty list is a success: without `--json`, it prints a short sentence pointing at the next step (loosen the filters, or `ito new`); with `--json`, it prints `[]`.
...
| `ito batch list`             | Lists the Project's Batches newest-first, each with its `created` date and derived progress (members `done`/total, current Wave — or a `waiting` marker when every open member is gated outside the Batch). |
| `ito batch show <name>`      | The Batch's open members grouped by **derived Waves**, followed by a non-numbered waiting group for members gated by non-`done` Issues outside the Batch, plus progress. `--include-done` includes completed members and marks historical Waves as done. A dependency cycle among members is a clear error naming the Issues involved. |
...
- **First run (decision 0005):** on a machine with no config file, bare `ito` on a TTY asks once whether to start empty or connect to an existing Ledger (URL + token, the same path as `ito ledger connect`), then opens the Digest; either answer writes the config so the question never repeats. Outside a TTY with no config it fails with exit `1` and an error on stderr naming `ito ledger connect`, instead of waiting on stdin. This is the only prompt in ito: every other command treats a missing config as local-only and never asks.
...
- **Batches `[2]` (extension, settled 2026-06-12):** a surface beside the Digest in the header tabs — **one screen, no drill-in**. Each Batch **with open work** renders as a Digest-style section (focus bar, name, derived progress, its `created` date dim at the right end of the rule), newest-first; its open members group under quiet **Wave** sub-headings (`WAVE n · READY/WAITING`), members gated outside the Batch follow in their own non-numbered waiting group, and done members live in the heading's progress count. Rows are Digest rows (priority mark, id, title, `⊘` group and labels right-aligned) led by a status mark (`◌` backlog, `○` todo, `◔` in progress, `◕` in review — a disc filling up as the work moves), since grouping by Wave drops the Status sections the Digest reads it from — so a READY Wave shows which members are already in flight and which are still free to start; each Wave heading counts its members per status behind the same marks, doubling as the legend, and drops that split when the frame is too narrow; a `conflicts_with` partner shows as a second `⊘` in its own colour next to the blocked marker. Same selection/focus model as the Digest (`tab` focuses a Batch, `↑↓` selects, `h` hides the focused Batch), same minimal edits (`s`, `p`, `l`), same `/` filter; `enter` opens the Issue detail. It reuses the section machinery the Digest already pays for; the genuinely new cost is the wave derivation, which lives in the core and also feeds the CLI (§6.3). Assigning Issues to Batches stays **out of the v2 TUI** (CLI only) — membership editing joins title/body/links in v3.
...
- **Keeping the Issue detail readable (settled 2026-09-23):** Links accumulate on an Issue the way Batches do on the surface, and they sit above the body, so they must cost a constant too. The Links render as **one block capped at four lines**, `↑/↓ N more` markers included: strongest kind first (`blocked by`, `conflicts with`, `relates to`), each linked Issue once under its strongest kind, the kind's label heading its group. The block is the detail's second focus stop — `tab` reaches it, `↑↓` selects a Link, `enter` opens the linked Issue — and the body stays the default focus, so `↑↓` still steps prev/next until you ask for the Links. `esc` unwinds one step at a time: block → body → back along the followed Links (landing on the Link that was followed) → the surface; prev/next navigation or opening from a surface starts a fresh trail. Every label shares one gutter, and Link rows truncate to the frame like every other row.
