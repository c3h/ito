...
**Status group**: The internal meaning of a set of **Status** values: _open_ (`backlog`, `todo`), _in-flight_ (`in_progress`, `in_review`), _closed_ (`done`). Reading vocabulary only — there is no column and no filter of its own; the views test the Status directly. _Avoid_: category (that's the field below), group, status type.

**Category**: A field saying what kind of work the Issue is: `uncategorized`, `bug`, `enhancement`, `docs`, `tests`, `refactor`, `chore`, `research`, `infra`. Agent-readable metadata, independent of **Status**, **Priority** and **Label**. _Avoid_: type, status group.
