...
- **Gains:** one definition of the domain types and the write logic; the TUI and CLI cannot drift. The exit-code leak in project resolution dissolves — the store returns typed errors and each consumer presents failure its own way (CLI → exit code + sentence; TUI → an error view).
- **Error mapping is many-to-one, and `ErrDetached` carries data.** Today `resolveProject` returns both "no Project for this directory" and "a Project exists but is detached from it" as the same exit `4` with *different* actionable sentences (the detached one names the Project, for `ito init --reattach <name>`). So detachment needs a **structured** error carrying that name: `ErrDetached` stays a sentinel to match against, and `DetachedError` wraps it with the Project name. The actionable wording is built at the CLI boundary, not inside the store. The store→exit-code mapping is many-to-one (`ErrNotRegistered` and `ErrDetached` both → `4`); this is the spot the extraction most easily breaks.
